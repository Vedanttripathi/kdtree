...
public class KdTree {
    ...
    private Node put(Node node, Point2D point, boolean isVertical, Node previous) {
        if (node == null) {
            sizecount++;
            return new Node(point, newNodeRec(point, previous), isVertical);
        }

        if (node.p.compareTo(point) == 0) return node;

        if (node.compareTo(point) > 0) {
            node.lb = put(node.lb, point, !isVertical, node);
        } else {
            node.rt = put(node.rt, point, !isVertical, node);
        }
    ...
    private RectHV newNodeRec(Point2D point, Node root) {
        ...
        if (root.split && cmp > 0)
            return new RectHV(root.rect.xmin(), root.rect.ymin(), root.p.x(), root.rect.ymax());
        else if (root.split && cmp < 0)
        ...
        else if (root.p.x() == point.x())
            return new RectHV(root.p.x(), root.rect.ymin(), root.rect.xmax(), root.rect.ymax());
        else return new RectHV(root.rect.xmin(), root.p.y(), root.rect.xmax(), root.rect.ymax());
